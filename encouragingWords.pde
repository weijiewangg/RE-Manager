...
// aligns text in screen
void createText(String text, float tSize, float margin) {
  textSize(tSize); // initializes text size
  String curText = ""; // current text line in screen
  ArrayList<String> textLines = new ArrayList<String>(); // arraylist of text lines
  
  // checks each quote to build text lines
  for (int i = 0; i < text.length(); i++)  {     
    curText += text.substring(i, i+1); // adds character to current text line
    
    if (textWidth(curText) > (width - margin - tSize)) {
      int l = curText.length();
      String curTextRev = "";
      
      for (int j = 0; j < l; j++) {
        curTextRev += curText.substring(l-j-1, l-j);
      }

      int lastSpace = l - curTextRev.indexOf(" ");
      String addText = curText.substring(0, lastSpace);
      textLines.add(addText);
      curText = curText.substring(lastSpace, l);
    }
  ...
  float tHeight = height/2 - 50*textLines.size();
  
  for (int i = 0; i < textLines.size(); i++) {
    text(textLines.get(i), margin, tHeight);
    tHeight += 100;
  }
}
